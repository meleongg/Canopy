...
import { ensureUnderstoryClosing, understoryPersonas } from "@/lib/understory";
...
export async function POST(request: Request) {
  ...
  const closingInstruction =
    userTurns.length === 5
      ? "FINAL RESPONSE CONTRACT: this is the learner's fifth and final turn. Answer any direct point in their message, then warmly recap or reinforce useful vocabulary and close the scene. Do not ask, invite, offer, or imply a follow-up. Do not use a question mark or Chinese question mark."
  ...
  const generation = {
    ...
    temperature: userTurns.length === 5 ? 0.2 : 0.7,
    system,
    ...conversation,
  };
